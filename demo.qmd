...
These units are based on infrastructure data (street & rail network the former two, buildings the latter). In addition, `rcrisp` also makes use of digital terrain data to estimate the river **valley**, the edges of which are used to "guide" the delineation of the river corridor.

## Data sources

By default, `rcrisp` uses the following data sources:

-   [OpenStreetMap](https://www.openstreetmap.org/) (OSM), mostly for street & railway network, river geometry, buildings;
...
Let's start by defining the name of a city and of a river which we aim to delineate:
...
for (i in seq(nrow(city_river_names))) {
  ...
  # Run the delineation for the selected city and river
  ...
  # Extract corridor, tranform to lat/lon
  corridor <- st_transform(delineations_city$corridor, "EPSG:4326")

  # Record corridor for later use
...
We thus quantify the green surface, and divide by the overall corridor surface:
